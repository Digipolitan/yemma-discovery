...
class YemmaDiscovery {
    ...
    _initIO() {
        ...
        this.io_client.on('connect', () => {
            console.log('YR > connected to registry with id:', this.io_client.id);
            if (this.options.subscribe === false)
                return;

            this.io_client.emit('/registry/subscribe', this.instance, token => this.access_token = token)
        });
        this.io_client.on('reconnect_attempt', attemptNumber => {
            console.warn('YR >  reconnect attempt', attemptNumber);
            this.io_client.opts.transports = ['polling', 'websocket'];
        });

        this.io_client.on('reconnect_failed', error => console.error('YR > reconnect_failed', error));
        this.io_client.on('reconnect_error', error => console.error('YR > reconnect_error', error));
        this.io_client.on('connect_error', error => console.error('YR > connect_error', error));
        this.io_client.on('connect_timeout', timeout => console.error('YR > connect_timeout', timeout));
        this.io_client.on('error', error => console.error('YR > Unknown error', error));
    }
